...
Generated: 2026-06-20T06:25:10.289225
Strategy: single_shot
Dataset: sample_claims.csv (20 rows)
Run dir: /Users/rishipanchal/Downloads/Multi-Modal Evidence Review/hackerrank-orchestrate-june26/results/run_20260620_062445_eval
...
| Field | Score |
|---|---|
| claim_status | 70.0% (Exact match) |
| issue_type | 55.0% (Exact match) |
| object_part | 95.0% (Exact match) |
| evidence_standard_met | 85.0% (Exact match) |
| valid_image | 85.0% (Exact match) |
| severity | 40.0% (Exact match) |
| risk_flags | 61.3% (Jaccard) |

## Operational Analysis

### Model Calls
- Rows processed: 20
- Measured LLM calls (sample): 20  (~1.0 per row)
- Pipeline: extract_claim (text) + analyze_images (vision) + synthesize_decision (text)
- Extrapolated to full test set (44 rows): ~44 calls

### Token Usage (measured)
- Total input tokens (sample): 15,545
- Total output tokens (sample): 2,875
- Average input tokens per call: ~777 (includes base64 image data for vision calls)
- Average output tokens per call: ~144
- Full test set extrapolation: ~34,199 input / ~6,325 output
...
~34,199 in / ~6,325 out on a paid tier
(e.g. Gemini 2.5 Flash @ $0.30/$2.50 per 1M) would cost roughly
$0.026.

### Latency / Runtime
- Sample run elapsed: 25.2s
- Average per row: 1.3s
- Full test set estimate: ~55s (~0.9 min)
